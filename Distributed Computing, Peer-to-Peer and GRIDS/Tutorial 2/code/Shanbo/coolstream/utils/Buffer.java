...
public class Buffer {
    ...
    private List<Integer> missedSegmentsDetail = new ArrayList<Integer>();
    ...
    public void countNumOfMissedSegments() {
        if (!this.buffer.contains(this.playbackPoint - 1)) {
            this.missedSegments++;
            missedSegmentsDetail.add(this.playbackPoint - 1);
        }
    }
    ...
    public String missedSegmentString(){
        String str = new String("[");
        Integer segment;
        Iterator<Integer> bufferIter = this.missedSegmentsDetail.iterator();
